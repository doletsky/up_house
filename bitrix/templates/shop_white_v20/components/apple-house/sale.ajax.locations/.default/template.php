...
<?if (count($arResult["REGION_LIST"]) > 0):?>
	...
	<select class="product-select" <?=$id?> <?if($disabled) echo "disabled";?> name="<?=$arParams["REGION_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>" onChange="<?=$change?>" type="location">
		<option class="product-item"><?echo GetMessage('SAL_CHOOSE_REGION')?></option>
		<?foreach ($arResult["REGION_LIST"] as $arRegion):?>
			<option class="product-item" value="<?=$arRegion["ID"]?>"<?if ($arRegion["ID"] == $arParams["REGION"]):?> selected="selected"<?endif;?>><?=$arRegion["NAME_LANG"]?></option>
		<?endforeach;?>
	</select>
<?endif;?>
...
<?if (count($arResult["CITY_LIST"]) > 0):?>
	...
	<select class="product-select" <?=$id?> <?if($disabled) echo "disabled";?> name="<?=$arParams["CITY_INPUT_NAME"]?>"<?if (strlen($arParams["ONCITYCHANGE"]) > 0):?> onchange="$('input[name=DELIVERY_ID]').eq(0).attr('checked', 'checked'); <?=$arParams["ONCITYCHANGE"]?>"<?endif;?> type="location">
		<option class="product-item" disabled="disabled"><?echo GetMessage('SAL_CHOOSE_CITY')?></option>
		<?foreach ($arResult["CITY_LIST"] as $arCity):?>
			<option class="product-item" value="<?=$arCity["ID"]?>"<?if ($arCity["ID"] == $arParams["CITY"]):?> selected="selected"<?endif;?>><?=($arCity['CITY_ID'] > 0 ? $arCity["CITY_NAME"] : GetMessage('SAL_CHOOSE_CITY_OTHER'))?></option>
		<?endforeach;?>
	</select>
    </div>
<?endif;?>
