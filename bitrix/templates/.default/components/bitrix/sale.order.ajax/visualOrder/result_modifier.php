...
if($arResult["ORDER_ID"]) {
	...
	if($arResult['ORDER']['PAY_SYSTEM_ID'] == 9) { //KupiVKredit & PayU
		$commisionProductID = 3986;
		$commisionPrice = number_format(($arResult['ORDER']['PRICE'] - $arResult['ORDER']['PRICE_DELIVERY'])*0.05, 2, '.', '');
		$commisionName = "Комиссия банка";
	}
	elseif($arResult['ORDER']['PAY_SYSTEM_ID'] == 7) { //KupiVKredit & PayU
		$commisionProductID = 3986;
		$commisionPrice = number_format(($arResult['ORDER']['PRICE'] - $arResult['ORDER']['PRICE_DELIVERY'])*0.039, 2, '.', '');
		$commisionName = "Комиссия платежной системы";
	}
	else {
