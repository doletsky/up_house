...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
    ...
    while($arSection = $rsSections->GetNext())
    {
        ...
        {
            $arResult['SECTIONS']['TOP'][$arSection['ID']]=$arSection;
        }
        else
        {
            $arResult['SECTIONS']['CHILD'][$arSection['IBLOCK_SECTION_ID']][$arSection['ID']]=$arSection;
        }
    }

    $childsLength = 0;
    foreach($arResult['SECTIONS']['CHILD'] as $childs)
    {
        $childsLength += count($childs);
    }

    $arResult["SECTIONS_COUNT"] = count($arResult['SECTIONS']['TOP']) + $childsLength;
