...
<?if (count($arResult["CITY_LIST"]) > 0):?>
    ...
    <div class="dropdown">
	<select <?=$id?> <?if($disabled) echo "disabled";?> name="<?=$arParams["CITY_INPUT_NAME"]?>"<?if (strlen($arParams["ONCITYCHANGE"]) > 0):?> onchange="$('input[name=DELIVERY_ID]').eq(0).attr('checked', 'checked'); <?=$arParams["ONCITYCHANGE"]?>"<?endif;?> type="location">
		<option disabled="disabled"><?echo GetMessage('SAL_CHOOSE_CITY')?></option>
