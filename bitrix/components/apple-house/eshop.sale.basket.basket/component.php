...
if (strlen($_REQUEST["BasketRefresh"]) > 0 || strlen($_REQUEST["BasketOrder"]) > 0 || strlen($_REQUEST["action"]) > 0)
{
	if(strlen($_REQUEST["action"]) > 0)
	{
		...
		if($id > 0)
		{
			$dbBasketItems = CSaleBasket::GetList(
					...
					false,
					false,
					array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "CURRENCY", "PROP")
				);
			if($arBasket = $dbBasketItems->Fetch())
			{
				if($_REQUEST["action"] == "delete" && in_array("DELETE", $arParams["COLUMNS_LIST"]))
				{
					$dbProp = CSaleBasket::GetPropsList(array("SORT" => "ASC", "ID" => "ASC"), array("BASKET_ID" => $id, "CODE" => array("OPTIONS")));
					while($arProp = $dbProp -> GetNext()){
						CSaleBasket::Delete($arProp["VALUE"]);
					}
	...
	{
		if ($arParams["HIDE_COUPON"] != "Y" && $_REQUEST['ApplyCoupon'] == 'Y')
		{
			$COUPON = Trim($_REQUEST["COUPON"]);
			if (strlen($COUPON) > 0) {
				if(CCatalogDiscount::SetCoupon($COUPON)) {
					CCatalogDiscount::SetCoupon('CP-S1UDN-704AAIY'); // купон на аксессуары
				}
				else {
					$arResult['COUPON_ERROR'] = true;
				}
			}
	...
	if (strlen($_REQUEST["BasketOrder"]) > 0 && $_REQUEST["ApplyCoupon"] != 'Y')
	{
