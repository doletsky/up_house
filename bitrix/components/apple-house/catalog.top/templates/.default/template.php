...
<table cellpadding="0" cellspacing="0" border="0">
	<?foreach($arResult["ROWS"] as $arItems):?>
        ...
        <? foreach($arItems as $arElement):?>
            <?//var_dump($arParams["ELEMENT_COUNT"]);
            ?>
        <? if($arElement["PRICE_MATRIX"]["MATRIX"][1][0]["PRICE"] == 0)
                continue;
            if($arParams["ELEMENT_COUNT"] < ++$iCounter)
                break;
        ...
        if($obElement = $rsElement->GetNextElement()){
            ...
            $arElement["DETAIL_PAGE_URL"] = '/' . $arProps['CML2_CODE']['VALUE'];
        }
        ?>
        <?
		...
		?>
			<td width="<?=$arResult["TD_WIDTH"]?>" id="<?=$this->GetEditAreaId($arElement['ID']);?>" <?=($arParams["VISIBLE_ELEMENT_COUNT"] <= $iCounter ? 'style="display: none;"' : '' )?>>
				<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td valign="top">
		...
		<?if($arResult["bDisplayPrices"]):?>
			<tr valign="top">
				...
				<?if(is_array($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):?>
					<td width="<?=$arResult["TD_WIDTH"]?>" class="data-cell" rowspan="2" <?=($arParams["VISIBLE_ELEMENT_COUNT"] <= $iCounter ? 'style="display: none;"' : '' )?>>
				...
				<?else:?>
					<td width="<?=$arResult["TD_WIDTH"]?>" class="data-cell" <?=($arParams["VISIBLE_ELEMENT_COUNT"] <= $iCounter ? 'style="display: none;"' : '' )?>>
		...
		<?if($arResult["bDisplayButtons"]):?>
			...
			<?if(is_array($arElement) && (!is_array($arElement["OFFERS"]) || empty($arElement["OFFERS"]))):?>
				<td <?=($arParams["VISIBLE_ELEMENT_COUNT"] <= $iCounter ? 'style="display: none;"' : '' )?>>
				<?if($arParams["DISPLAY_COMPARE"]):?>
					<noindex>
				...
				<?if($arElement["CAN_BUY"]):?>
					...
					<?else:?>
						<noindex>
                            <a href="#" class="b_button button_green" data-product-id="<?=$arElement["ID"]?>">
                                <i class="b_glyph glyph_cart-mini"></i> <?echo GetMessage("CATALOG_BUY")?>
                            </a>

                            <? /*
                        <a href="<?echo $arElement["BUY_URL"]?>#accessories" class="b_button button_green" onclick="_gaq.push(['_trackEvent', 'Acc tab', 'Buy', '']);">
                            <i class="b_glyph glyph_cart-mini"></i> <?echo GetMessage("CATALOG_BUY")?>
                        </a>
                            */?>


						<?/* <a href="<?echo $arElement["BUY_URL"]?>#accessories" rel="nofollow"><?echo GetMessage("CATALOG_BUY")?></a>
